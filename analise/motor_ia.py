...
_PROMPT_PARECER = (
    ...
    "O que cada status significa (a triagem mede a PRECISÃO do detector, então "
    "prefira SEMPRE uma decisão terminal — Confirmado ou Descartado):\n"
    "- Confirmado = o indício se SUSTENTA com os dados presentes; é um verdadeiro-"
    "positivo do detector, um alerta legítimo que merece fiscalização. NÃO exige "
    "provar irregularidade ou crime — basta que o padrão seja real e não tenha "
    "explicação de rotina (ex.: valor de fato atípico para o objeto, dispensa "
    "realmente questionável, concentração de fornecedor real).\n"
    "- Descartado = FALSO-positivo; há explicação natural nos próprios dados (valor "
    "rotineiro para o setor — energia, folha, merenda, concessão —, categoria/objeto "
    "que não se aplica ao detector, ou dados inconsistentes).\n"
    "- Investigando = use APENAS quando os dados do JSON forem genuinamente "
    "insuficientes para decidir entre Confirmado e Descartado, exigindo uma fonte "
    "externa que não está aqui. NÃO use como default cauteloso.\n"
    "- Aberto = falta contexto mínimo até para começar.\n\n"
    "Coerência obrigatória:\n"
    "- 'Provável problema' -> Confirmado (indício se sustenta) — só use Investigando "
    "se faltar um dado essencial que não está no JSON.\n"
    "- 'Provavelmente explicável' -> Descartado.\n"
    "- 'Inconclusivo' -> Investigando.\n"
    "- Descartar exige que o indício esteja EXPLICADO, sem pendências: se a Análise "
    "ainda recomenda confirmar/verificar/checar algo antes de concluir, NÃO é "
    "Descartado. Nesse caso, se o indício aponta problema, use Confirmado; se for "
    "mesmo incerto, Investigando.\n"
