...
| 5.2 | **API pública documentada** ✅ | OpenAPI 3.0 + Swagger UI em `/api/docs`; `/api/v1/` versionado para alertas, contratos e precisão (leitura, CORS liberado); rate limit por IP (flask-limiter, `Fly-Client-IP`) com cabeçalhos `X-RateLimit-*` e `429` em JSON | S–M | Alto — jornalistas de dados citam quem os serve |
